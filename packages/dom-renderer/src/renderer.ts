...
import type { GridEngine, ColumnState, RowNode } from '@gridstorm/core';
...
import type { HeaderRendererHost } from './header-renderer';
...
export class DomRenderer {
  ...
  // Column header rendering controller (leaf + group header rows)
  ...
  constructor(config: DomRendererConfig) {
    ...
    // Column header rendering is delegated to a dedicated controller that talks
    // to the renderer only through this narrow host surface.
    const headerHost: HeaderRendererHost = {
      prefix: this.prefix,
      checkboxSelection: this.checkboxSelection,
      groupHeaderHeight: this.groupHeaderHeight,
      getState: () => this.engine.store.getState(),
      getHeaderHeight: () => this.engine.api.getGridOption('headerHeight') ?? 48,
      getScrollLeft: () => this.bodyViewport?.scrollLeft ?? 0,
      getVisibleColumnsForRender: (scrollLeft) => this.getVisibleColumnsForRender(scrollLeft),
      setLastColumnResult: (result) => {
        this.lastColumnResult = result;
      },
      isColumnVirtualized: () => this.columnVirtualizer.isVirtualized(),
      getColumnTotalWidth: () => this.columnVirtualizer.getTotalWidth(),
      el: (tag, className) => this.el(tag, className),
      getHeaderContainer: () => this.headerContainer,
      getRoot: () => this.root,
      createCheckboxHeaderCell: () => this.createCheckboxHeaderCell(),
      dispatch: (command, payload) => this.engine.commandBus.dispatch(command, payload),
      rerenderFloatingFilter: () => {
        if (this.extFloatingFilter) {
          this.extFloatingFilter.rerender(this.buildContext());
        }
      },
      emitHeaderRendered: () => this.engine.eventBus.emit('dom:headerRendered', {}),
    };
    this.headerRenderer = new HeaderRenderer(headerHost);
  }
  ...
  // Column header DOM construction (leaf header row, multi-level column group
  // header rows, per-column header cells with sort handling) lives in
  // HeaderRenderer (./header-renderer.ts). This thin delegator preserves the
  // many internal call sites that trigger a header rebuild.
  ...
  private createCell(
    ...
    const tooltipField = col.originalDef ? (col.originalDef as any).tooltipField : undefined;
